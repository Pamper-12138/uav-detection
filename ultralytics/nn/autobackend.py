...
class AutoBackend(nn.Module):
    """
    Handles dynamic backend selection for running inference using Ultralytics YOLO yolo_models.
    ...
    yolo_models across various platforms.
    """
    ...
    def forward(self, im, augment=False, visualize=False, embed=None):
        ...
        # CoreML
            ...
            if "confidence" in y:
                raise TypeError(
                    "Ultralytics only supports inference of non-pipelined CoreML yolo_models exported with "
            ...
            if len(y) == 2 and len(y[1].shape) != 4:  # segmentation model
                y = list(reversed(y))  # reversed for segmentation yolo_models (pred, proto)
            ...
            if self.saved_model:  # SavedModel
                ...
                for output in self.output_details:
                    ...
                    if x.ndim == 3:  # if task is not classification, excluding masks (ndim=4) as well
                        ...
                        # xywh are normalized in TFLite/EdgeTPU to mitigate quantization error of integer yolo_models
