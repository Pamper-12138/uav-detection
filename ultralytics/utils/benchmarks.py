    ...
    for i, (name, format, suffix, cpu, gpu, _) in enumerate(zip(*export_formats().values())):
        ...
        try:
            ...
            if i == 11:  # Paddle
                ...
                assert not is_end2end, "End-to-end yolo_models not supported by PaddlePaddle yet"
            ...
            if i == 15:  # RKNN
                ...
                assert not is_end2end, "End-to-end yolo_models not supported by RKNN yet"
...
class ProfileModels:
    """
    ProfileModels class for profiling different yolo_models on ONNX and TensorRT.

    This class profiles the performance of different yolo_models, returning results such as model speed and FLOPs.

    Attributes:
        paths (List[str]): Paths of the yolo_models to profile.
        num_timed_runs (int): Number of timed runs for the profiling.
        num_warmup_runs (int): Number of warmup runs before profiling.
        min_time (float): Minimum number of seconds to profile for.
        imgsz (int): Image size used in the yolo_models.
        half (bool): Flag to indicate whether to use FP16 half-precision for TensorRT profiling.
        trt (bool): Flag to indicate whether to profile using TensorRT.
        device (torch.device): Device used for profiling.

    Methods:
        profile: Profiles the yolo_models and prints the result.

    Examples:
        Profile yolo_models and print results
    ...
    def __init__(
        ...
        """
        Initialize the ProfileModels class for profiling yolo_models.

        Args:
            paths (List[str]): List of paths of the yolo_models to be profiled.
            num_timed_runs (int): Number of timed runs for the profiling.
        ...
        Examples:
            Initialize and profile yolo_models
            >>> from ultralytics.utils.benchmarks import ProfileModels
            >>> profiler = ProfileModels(["yolo11n.yaml", "yolov8s.yaml"], imgsz=640)
            >>> profiler.profile()
        """
    ...
    def profile(self):
        """Profiles YOLO yolo_models for speed and accuracy across various formats including ONNX and TensorRT."""
