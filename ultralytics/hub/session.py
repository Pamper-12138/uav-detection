...
class HUBTrainingSession:
    """
    HUB training session for Ultralytics HUB YOLO yolo_models. Handles model initialization, heartbeats, and checkpointing.
    ...
    def __init__(self, identifier):
        ...
        try:
            ...
            if identifier.startswith(f"{HUB_WEB_ROOT}/yolo_models/") and not self.client.authenticated:
                LOGGER.warning(
    ...
    def create_session(cls, identifier, args=None):
        ...
        try:
            ...
            if args and not identifier.startswith(f"{HUB_WEB_ROOT}/yolo_models/"):  # not a HUB model URL
    ...
    def load_model(self, model_id):
        ...
        self.model_url = f"{HUB_WEB_ROOT}/yolo_models/{self.model.id}"
    ...
    def _parse_identifier(identifier):
        ...
        if Path(identifier).suffix in {".pt", ".yaml"}:
            filename = identifier
        elif identifier.startswith(f"{HUB_WEB_ROOT}/yolo_models/"):
            ...
            raise HUBModelError(f"model='{identifier} invalid, correct format is {HUB_WEB_ROOT}/yolo_models/MODEL_ID")
