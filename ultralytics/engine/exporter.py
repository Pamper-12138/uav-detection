...
class Exporter:
    ...
    def __call__(self, model=None) -> str:
        ...
        if self.args.int8 and tflite:
            assert not getattr(model, "end2end", False), "TFLite INT8 export not supported for end2end yolo_models."
        if self.args.nms:
            assert not isinstance(model, ClassificationModel), "'nms=True' is not valid for classification yolo_models."
            if getattr(model, "end2end", False):
                LOGGER.warning("WARNING ⚠️ 'nms=True' is not available for end2end yolo_models. Forcing 'nms=False'.")
                self.args.nms = False
        ...
        if isinstance(model, WorldModel):
            LOGGER.warning(
                ...
                "WARNING ⚠️ YOLOWorldv2 yolo_models (i.e. 'yolov8s-worldv2.pt') only support export to "
    ...
    def get_int8_calibration_dataloader(self, prefix=""):
        """Build and return a dataloader suitable for calibration of INT8 yolo_models."""
    ...
    def export_coreml(self, prefix=colorstr("CoreML:")):
        ...
        if self.model.task == "classify":
            ...
            if self.args.nms:
                LOGGER.warning(f"{prefix} WARNING ⚠️ 'nms=True' is only available for Detect yolo_models like 'yolo11n.pt'.")
    ...
    def export_saved_model(self, prefix=colorstr("TensorFlow SavedModel:")):
        ...
        # Remove/rename TFLite yolo_models
        if self.args.int8:
    ...
    def export_imx(self, prefix=colorstr("IMX:")):
        ...
        if getattr(self.model, "end2end", False):
            raise ValueError("IMX export is not supported for end2end yolo_models.")
        if "C2f" not in self.model.__str__():
            raise ValueError("IMX export is only supported for YOLOv8n detection yolo_models")
    ...
    def _add_tflite_metadata(self, file):
        """Add metadata to *.tflite yolo_models per https://www.tensorflow.org/lite/models/convert/metadata."""
        import flatbuffers
    ...
    def _pipeline_coreml(self, model, weights_dir=None, prefix=colorstr("CoreML Pipeline:")):
        ...
        # 4. Pipeline yolo_models together
