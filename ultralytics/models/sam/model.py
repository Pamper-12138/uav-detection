...
class SAM(Model):
    ...
    Attributes:
        ...
        task (str): The task type, set to "segment" for SAM yolo_models.
    ...
    def task_map(self):
        ...
        Returns:
            (Dict[str, Type[Predictor]]): A dictionary mapping the 'segment' task to its corresponding Predictor
                class. For SAM2 yolo_models, it maps to SAM2Predictor, otherwise to the standard Predictor.

        Examples:
            >>> sam = SAM("sam_b.pt")
            >>> task_map = sam.task_map
            >>> print(task_map)
            {'segment': <class 'ultralytics.yolo_models.sam.predict.Predictor'>}
        """
